...
(function($) {
    $.fn.lazyVideoLoader = function(options) {
        ...
        for (let state in visibilityStates) {
            if (typeof document[visibilityStates[state][0]] !== "undefined") {
                [hidden, visibilityChange] = visibilityStates[state];
                break;
            }
        }
        ...
        function loadVideos() {
            this.each((index, videoElement) => {
                $(videoElement).attr('id', `video-${index}`);
                observer.observe(videoElement);
                document.addEventListener(visibilityChange, () => handleVisibilityChange(videoElement), false);
            });
        }
        ...
        function setupPlayButton(overlay, video) {
            ...
            overlay.html(playButtonTemplate);
            video.on('loadedmetadata', () => video.prop('controls', false));
            overlay.find('.play-button').click(() => {
                video[0].play();
            ...
            video.on('ended', () => {
                video.removeAttr('data-user-started');
                overlay.show();
                video.prop('controls', false);
            });
        }
